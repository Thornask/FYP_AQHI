...
temp_all$station[temp_all$station == "中西區"] <- 'CentralWest'
temp_all$station[temp_all$station == "東區"] <- 'Eastern'
temp_all$station[temp_all$station == "葵涌"] <- 'KwaiChung'
temp_all$station[temp_all$station == "觀塘"] <- 'KwunTong'
temp_all$station[temp_all$station == "北區"] <- 'North'
temp_all$station[temp_all$station == "銅鑼灣"] <- 'roadside_CausewayBay'
temp_all$station[temp_all$station == "中環"] <- 'roadside_Central'
temp_all$station[temp_all$station == "旺角"] <- 'roadside_MongKok'
temp_all$station[temp_all$station == "深水埗"] <- 'ShamShuiPo'
temp_all$station[temp_all$station == "沙田"] <- 'ShaTin'
temp_all$station[temp_all$station == "南區"] <- 'Southern'
temp_all$station[temp_all$station == "大埔"] <- 'TaiPo'
temp_all$station[temp_all$station == "塔門"] <- 'TapMun'
temp_all$station[temp_all$station == "將軍澳"] <- 'TseungKwanO'
temp_all$station[temp_all$station == "荃灣"] <- 'TsuenWan'
temp_all$station[temp_all$station == "屯門"] <- 'TuenMun'
temp_all$station[temp_all$station == "東涌"] <- 'TungChung'
temp_all$station[temp_all$station == "元朗"] <- 'YuenLong'

# separate to different station (for testing if need)
CentralWest <- subset(temp_all, temp_all$station == 'CentralWest')
Eastern <- subset(temp_all, temp_all$station == 'Eastern')
# KwaiChung <- subset(temp_all, temp_all$station == 'KwaiChung')
# KwunTong <- subset(temp_all, temp_all$station == 'KwunTong')
# North <- subset(temp_all, temp_all$station == 'North')
roadside_CausewayBay <- subset(temp_all, temp_all$station == 'roadside_CausewayBay')
# roadside_Central <- subset(temp_all, temp_all$station == 'roadside_Central')
# roadside_MongKok <- subset(temp_all, temp_all$station == 'roadside_MongKok')
# ShamShuiPo <- subset(temp_all, temp_all$station == 'ShamShuiPo')
# ShaTin <- subset(temp_all, temp_all$station == 'ShaTin')
# Southern <- subset(temp_all, temp_all$station == 'Southern')
# TaiPo <- subset(temp_all, temp_all$station == 'TaiPo')
TapMun <- subset(temp_all, temp_all$station == 'TapMun')
# TseungKwanO <- subset(temp_all, temp_all$station == 'TseungKwanO')
# TsuenWan <- subset(temp_all, temp_all$station == 'TsuenWan')
# TuenMun <- subset(temp_all, temp_all$station == 'TuenMun')
# TungChung <- subset(temp_all, temp_all$station == 'TungChung')
# YuenLong <- subset(temp_all, temp_all$station == 'YuenLong')
...
temp_all <- temp_all %>% relocate(hour, .after = station)
temp_all$HKTdate <- temp_all$date
temp_all <- temp_all %>% relocate(HKTdate, .after = station)
...
## C(NO2), C(SO2), C(O3), C(PM10) 及 C(PM2.5) 是各污染物的三小時移動平均濃度(以微克/立方米計); 及
