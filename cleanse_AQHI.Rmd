...
# (further data)
second <- read_csv("C:\\Users\\user\\OneDrive\\Documents\\AQHI data\\in_station(07012022-01312023)\\air_hourly.csv")

# Combine older data as one dataset
...
colnames(second)[which(names(second) == "日期")] <- "date"
colnames(second)[which(names(second) == "小時")] <- "hour"
colnames(second)[which(names(second) == "監測站")] <- "station"

## ... and remove unnecessary value
all[, c("NOX", "CO")] <- list(NULL) 
## re-locate the column
all[, c('FSP', 'NO2', 'O3', 'RSP', 'SO2')] <- all[, c('NO2', 'SO2', 'O3', 'RSP', 'FSP')]
colnames(all)[-1:-3] <- c('NO2', 'SO2', 'O3', 'RSP', 'FSP')
second[, c('SO2', 'NO2', 'RSP', 'O3', 'FSP')] <- second[, c('NO2', 'SO2', 'O3', 'RSP', 'FSP')]
colnames(second)[-1:-3] <- c('NO2', 'SO2', 'O3', 'RSP', 'FSP')

## Combine all data as one dataset
temp_all <- rbind(all, second)
...
## Remove the data before years 2014
temp_all <- temp_all[as.numeric(format(temp_all$date, format = "%Y")) >= 2014, ]
...
for(sel_name in station_egName){
  ...
  sel_df <- na.locf(sel_df, fromLast = FALSE)
  assign(sel_name, sel_df)
}
rm(df, sel_df)
...
# subset sample data from station (archieved)
```{r}
...
# mongodb accessing (testing) (archieved)
```{r}
# install.packages("mongolite")
# require(mongolite)

# connection_string <- 'mongodb+srv://Thornask:xxx@cluster0.neeez8b.mongodb.net/test'
# centralwest_collection <- mongo(collection="CentralWest", db="dbTseries", url=connection_string)
# centralwest_collection$count()
# centralwest_collection$find('{"date": {"$gte": {"$date":"2023-05-12T15:00:00Z"}}}')
# centralwest_collection$find('{"date": {"$gte": {"$date":"2022-06-30T23:00:00Z"}}}')
# north_collection$find('{"date": {"$date":"2023-05-11T14:00:00Z"}}')
```
# draft
```{r}
#temp_all$date <- strsplit(temp_all$date, '/')

## (Remove the data before years 2014)
#temp_all <- temp_all[as.numeric(temp_all$date[[]]) >= 2014, ]

#for (i in 1:nrow(temp_all)){
#     temp_all$date[i] <- ISOdate(as.numeric(temp_all$date[[i]][3]), as.numeric(temp_all$date[[i]][2]), as.numeric(temp_all$date[[i]][1]), as.numeric(temp_all$hour[i]), 0, 0)
# }
```

### (debugging for mongodb access)
```{r}
debug <- mongo(collection="debug", db="debug", url=connection_string)
if(
...
```{r}
# a <- read_csv("C:\\Users\\user\\Desktop\\all_wo_NULL\\all_wo_NULL.csv")
```
