...
# (backup solution of insert data in case having missing data of the past 24 hours)
```{r}
...
table_CW <- na.locf(table_CW, fromLast = FALSE)

## reverse the order of the data
tempbp <- table_CW
tempbp <- tempbp %>% map_df(rev)

## code for formatting the date value
tempbp$split <- strsplit(tempbp$date, "")
tempbp$date <- as.Date(tempbp$date)
for (f in 1:24) {
  tempbp$hour[f] <- paste0(tempbp$split[[f]][12], tempbp$split[[f]][13]) %>% as.numeric()
}  
tempbp$date <- ISOdate(tempbp$date, tempbp$hour,0,0)

tempbp <- subset(tempbp, select = -c(hour, split))

CentralWest <- rbind(CentralWest, tempbp[15:24,])
write_excel_csv(CentralWest, "C:\\Users\\user\\OneDrive\\Documents\\AQHI data\\cleansed_dataset\\CentralWest.csv")

new_CentralWest <- read_csv("C:\\Users\\user\\OneDrive\\Documents\\AQHI data\\cleansed_dataset\\new_CentralWest.csv")
new_CentralWest <- rbind(new_CentralWest, tempbp[15:24,])
write_excel_csv(new_CentralWest, "C:\\Users\\user\\OneDrive\\Documents\\AQHI data\\cleansed_dataset\\new_CentralWest.csv")
...
table_KC <- table_KC[order(nrow(table_KC):1),]

table_KC <- na.locf(table_KC, fromLast = FALSE)
# table_KC[1:4,]

KwaiChung <- rbind(KwaiChung, table_KC)
...
table_SSP <- table_SSP[order(nrow(table_SSP):1),]

table_SSP <- na.locf(table_SSP, fromLast = FALSE)
# table_SSP[1:4,]

ShamShuiPo <- rbind(ShamShuiPo, table_SSP)
...
table_TAIPO <- table_TAIPO[order(nrow(table_TAIPO):1),]

table_TAIPO <- na.locf(table_TAIPO, fromLast = FALSE)
# table_TAIPO[1:4,]

TaiPo <- rbind(TaiPo, table_TAIPO)
...
table_TAPMUN <- table_TAPMUN[order(nrow(table_TAPMUN):1),]

table_TAPMUN <- na.locf(table_TAPMUN, fromLast = FALSE)
# table_TAPMUN[1:4,]

TapMun <- rbind(TapMun, table_TAPMUN)
...
table_TW <- table_TW[order(nrow(table_TW):1),]

table_TW <- na.locf(table_TW, fromLast = FALSE)
# table_TW[1:4,]

TsuenWan <- rbind(TsuenWan, table_TW)
...
table_TC <- table_TC[order(nrow(table_TC):1),]

table_TC <- na.locf(table_TC, fromLast = FALSE)
# table_TC[1:4,]

TungChung <- rbind(TungChung, table_TC)
...
table_YL <- table_YL[order(nrow(table_YL):1),]

table_YL <- na.locf(table_YL, fromLast = FALSE)
# table_YL[1:4,]

YuenLong <- rbind(YuenLong, table_YL)
...
table_CWB <- table_CWB[order(nrow(table_CWB):1),]

table_CWB <- na.locf(table_CWB, fromLast = FALSE)
# table_CWB[1:4,]

roadside_CausewayBay <- rbind(roadside_CausewayBay, table_CWB)
...
table_CENT <- table_CENT[order(nrow(table_CENT):1),]

table_CENT <- na.locf(table_CENT, fromLast = FALSE)
# table_CENT[1:4,]

roadside_Central <- rbind(roadside_Central, table_CENT)
...
table_SHATIN <- table_SHATIN[order(nrow(table_SHATIN):1),]

table_SHATIN <- na.locf(table_SHATIN, fromLast = FALSE)
# table_SHATIN[1:4,]

ShaTin <- rbind(ShaTin, table_SHATIN)
...
table_KT <- table_KT[order(nrow(table_KT):1),]

table_KT <- na.locf(table_KT, fromLast = FALSE)
# table_KT[1:4,]

KwunTong <- rbind(KwunTong, table_KT)
...
table_TKO <- table_TKO[order(nrow(table_TKO):1),]

table_TKO <- na.locf(table_TKO, fromLast = FALSE)
# table_TKO[1:4,]

TseungKwanO <- rbind(TseungKwanO, table_TKO)
...
table_EAST <- table_EAST[order(nrow(table_EAST):1),]

table_EAST <- na.locf(table_EAST, fromLast = FALSE)
# table_EAST[1:4,]

Eastern <- rbind(Eastern, table_EAST)
...
table_MK <- table_MK[order(nrow(table_MK):1),]

table_MK <- na.locf(table_MK, fromLast = FALSE)
# table_MK[1:4,]

roadside_MongKok <- rbind(roadside_MongKok, table_MK)
...
table_TUENMUN <- table_TUENMUN[order(nrow(table_TUENMUN):1),]

table_TUENMUN <- na.locf(table_TUENMUN, fromLast = FALSE)
# table_TUENMUN[1:4,]

TuenMun <- rbind(TuenMun, table_TUENMUN)
...
table_SOUTH <- table_SOUTH[order(nrow(table_SOUTH):1),]

table_SOUTH <- na.locf(table_SOUTH, fromLast = FALSE)
# table_SOUTH[1:4,]

Southern <- rbind(Southern, table_SOUTH)

temp_SOUTH <- table_SOUTH
temp_SOUTH <- na.locf(temp_SOUTH, fromLast = FALSE)
temp_SOUTH <- temp_SOUTH[order(nrow(temp_SOUTH):1),]
Southern <- rbind(Southern, temp_SOUTH)
...
table_NORTH <- table_NORTH[order(nrow(table_NORTH):1),]

table_NORTH <- na.locf(table_NORTH, fromLast = FALSE)
# table_NORTH[1:4,]

North <- rbind(North, table_NORTH)
